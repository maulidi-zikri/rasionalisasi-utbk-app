...
const totalTryoutsEl = document.getElementById('total-tryouts');
...
function init() {
    ...
    updateStats();
    renderChart();
    populateTryoutDropdown(); // Populate dropdown options

    setupEventListeners();
    setupCustomDropdown(); // Initialize custom dropdown listeners
    startCountdown();
    renderSchedule();
    renderCalendar();
    renderCalendar();
...
function switchTab(tabName) {
    const sections = ['home', 'schedule', 'ptn'];
    ...
    // This is a simple way to set active based on onclick attribute text
    // A more robust way would be to identify them by ID or data attribute
...
// Schedule Data with Date Objects for Calendar Logic
const scheduleData = [
    {
...
function getEventStatus(start, end) {
    ...
    if (endDate && today > endDate) {
        return { label: 'Selesai', class: 'status-ended' };
    } else if (today >= startDate && (!endDate || today <= endDate)) {
        return { label: 'Sedang Berjalan', class: 'status-ongoing' };
    } else {
...
function renderSchedule() {
    ...
    currentMonths.forEach(monthItem => {
        ...
        monthHeader.textContent = monthItem.month;
    ...
    for (let i = 1; i <= lastDate; i++) {
        ...
        if (event) {
            ...
            div.addEventListener('click', () => {
                // Find which page the month belongs to
                // We need to match currentMonth + currentYear with scheduleData
                const targetMonthIndex = scheduleData.findIndex(m => {
                    return m.month === `${monthNames[currentMonth]} ${currentYear}`;
                });
...
function startCountdown() {
    ...
    setInterval(() => {
        ...
        if (distance < 0) {
            document.getElementById('countdown').innerHTML = "Waktunya UTBK!";
            return;
        }
...
function setupEventListeners() {
    ...
    window.onclick = function (event) {
        ...
        if (altModal && event.target == altModal) {
            closeAltModal();
        }
...
function renderDropdownOptions(options) {
    ...
    options.forEach(opt => {
        ...
        if (existingScore) {
            ...
            div.innerHTML = `<span style="flex-grow: 1;">${opt.name}</span> <span style="font-size: 0.8rem; color: var(--primary-blue); font-weight: 600;">Skor Tryout: ${existingScore.score}</span>`;
        } else {
...
function closeModal() {
    ...
    editingScoreId = null;
    document.querySelector('.submit-btn').textContent = 'Simpan';
    document.getElementById('scoreForm').reset();

    // Check if dropdown elements exist before accessing property
    const selectedText = document.querySelector('.selected-text');
    if (selectedText) selectedText.textContent = 'Pilih Tryout';
...
function showNotification() {
    ...
    if (ongoingEvent) {
        ...
        notifBody.innerHTML = `
            <div class="notif-badge badge-ongoing">Event Berjalan</div>
            <div class="notif-icon-wrapper ongoing">
                <i class="fa-solid fa-bell"></i>
            </div>
            <div class="notif-title">${ongoingEvent.name}</div>
            <div class="notif-message">
                Kamu belum mengerjakan Tryout ini! Waktu pengerjaan tinggal <strong>${diffDays} hari lagi</strong> sebelum periode berakhir.
            </div>
            <button class="notif-action-btn" onclick="goToEventDetails('${ongoingEvent.name.replace(/'/g, "\\\'")}')">Kerjakan Sekarang</button>
        `;
        ...
        notifBody.innerHTML = `
            <div class="notif-badge badge-upcoming">Mendatang</div>
            <div class="notif-icon-wrapper">
                <i class="fa-solid fa-calendar-check"></i>
            </div>
            <div class="notif-title">${upcomingEvent.name}</div>
            <div class="notif-message">
                Tryout berikutnya akan tersedia dalam <strong>${diffDays} hari lagi</strong>. Siapkan diri kamu untuk hasil maksimal!
            </div>
            <button class="notif-action-btn" onclick="closeNotifModal()">Oke, Siap!</button>
        `;
    } else {
        notifBody.innerHTML = `
            <div class="notif-badge badge-upcoming">Selesai</div>
            <div class="notif-icon-wrapper">
                <i class="fa-solid fa-check-double"></i>
            </div>
            <div class="notif-title">Semua Terkejar!</div>
            <div class="notif-message">
                Hebat! Kamu telah mengikuti semua jadwal Tryout yang tersedia sejauh ini. Terus pantau jadwal untuk info terbaru.
            </div>
            <button class="notif-action-btn" onclick="closeNotifModal()">Mantap!</button>
        `;
    }
...
function handleFormSubmit(e) {
    ...
    if (name && date && !isNaN(score)) {
        ...
        // Reset dropdown UI
        // Reset dropdown UI
        document.querySelector('.selected-text').textContent = 'Pilih Tryout';
    ...
    if (scores.length === 0) {
        scoreListEl.innerHTML = `
            <div class="empty-state">
                <p>Belum ada data tryout. Yuk, masukkan nilai pertamamu!</p>
            </div>
        `;
        return;
    }
...
function calculateRationalization(avgScore, dt, peminat, universityName = '', minScoreData = null) {
    if (!avgScore || avgScore === 0) return { label: 'Belum ada nilai', class: 'unknown' };
    if (!dt || !peminat || peminat === 0) return { label: 'Tidak bisa diprediksi', class: 'unknown' };
    ...
    if (minScoreData === 'belum ada data' || minScoreData === 'No Data') {
        return { label: 'Tidak bisa diprediksi', class: 'unknown', target: 'Belum ada data' };
    }
    ...
    let chanceData = { label: 'Perlu Peningkatan', class: 'low', target: targetScore };
    if (diff >= 20) {
        chanceData = { label: 'Skor mencukupi', class: 'very-high', target: targetScore };
    } else if (diff >= 0) {
        chanceData = { label: 'Berpeluang', class: 'high', target: targetScore };
    }
...
function renderPtnSlider() {
    ...
    dreamPTNs.forEach((ptn, index) => {
        ...
        if (isFilled) {
            ...
            if (typeof masterDataPTN !== 'undefined') {
                ...
                if (uniData) {
                    ...
                    if (prodiData) {
                        ...
                        const dtDisplay = dt > 0 ? dt : '<span class="no-data">Belum ada data</span>';
                        const peminatDisplay = peminat > 0 ? peminat : '<span class="no-data">Belum ada data</span>';

                        let keketatanDisplay = '<span class="no-data">Belum ada data</span>';
                        if (dt > 0 && peminat > 0) {
                            const pct = (dt / peminat) * 100;
                            // Format to 2 decimal places if small, or integer if large enough? 
                            // Usually percentage is small e.g. 1.5%
                        ...
                        chanceHtml = `
                            <div style="margin-top: 8px;">
                                ...
                                <div style="font-size: 0.8rem; color: #666; margin-top: 4px;">
                                    Target Minimum Skor: <strong>${chance.target || '-'}</strong>
                                </div>
                            </div>
                        `;
                        ...
                        statsHtml = `
                            <div class="ptn-stats-grid">
                                <div class="stat-item fade-in-up" style="animation-delay: 0.1s;">
                                    ...
                                    <span class="stat-label">Daya Tampung</span>
                                ...
                                <div class="stat-item fade-in-up" style="animation-delay: 0.2s;">
                                    ...
                                    <span class="stat-label">Peminat</span>
                                ...
                                <div class="stat-item fade-in-up" style="animation-delay: 0.3s;">
                                    ...
                                    <span class="stat-label">Keketatan</span>
            ...
            card.innerHTML = `
                <button class="ptn-delete-btn" title="Hapus Pilihan" onclick="event.stopPropagation(); removePtnChoice(${index});">
                ...
                <div class="card-footer-info">
                    ${statsHtml || 'Ingin tau peluang lolos di prodi tujuanmu? Ikuti Tryout Ruangguru sekarang!'}
                </div>
            `;
        } else {
            ...
            let labelText = `Pilihan ${index === 0 ? 'Utama' : index === 1 ? 'Kedua' : index === 2 ? 'Ketiga' : 'Keempat'}`;
            let subText = '';

            if (index >= 2) {
                subText = '<div style="font-size: 0.7rem; margin-top: 4px; opacity: 0.8;">Harus Prodi D3/D4</div>';
            }
...
function renderChart() {
    ...
    scoreChart = new Chart(ctx, {
        ...
        options: {
            ...
            scales: {
                y: {
                    beginAtZero: true, // Optional: might want to start from min score
                    grid: {
                        display: false
                    },
                    ticks: {
...
document.addEventListener('DOMContentLoaded', () => {
    // Set default date
    document.getElementById('to-date').valueAsDate = new Date();
    calculateInitialPage(); // Determine which page to show first
    init();
});
    ...
    if (event.type === 'exam') {
        ...
        if (existingScore) {
            ...
            if (prevScores.length > 0) {
                ...
                trendEl.textContent = 'Tryout Pertama';
                trendEl.classList.add('trend-neutral');
            }
        }
        ...
        if (existingScore) {
            linkContainer.innerHTML = `<span class="detail-status-text detail-status-done">Sudah dikerjakan</span>`;
        } else {
            const status = getEventStatus(event.start, event.end);

            if (status.class === 'status-ended') {
                linkContainer.innerHTML = `<span class="detail-status-text detail-status-ended">Selesai</span>`;
            } else if (status.class === 'status-upcoming') {
                linkContainer.innerHTML = `<span class="detail-status-text detail-status-upcoming">Belum dimulai</span>`;
            } else {
                // Ongoing - Show Link
                ...
                linkContainer.innerHTML = `
                    <a href="${link}" target="_blank" class="detail-link-btn">
                        Mulai Kerjakan <i class="fa-solid fa-arrow-up-right-from-square"></i>
                    </a>
                `;
            }
        }
...
function openPtnModal(index = 0) { // Default to first slot if not specified, though usually clicked from specific card
    ...
    if (index >= 2) {
        modalTitle.textContent = `Pilih Universitas & Jurusan (Pilihan ${index + 1} - D3/D4)`;
    } else {
        modalTitle.textContent = "Pilih Universitas & Jurusan";
    }
...
function resetPtnModalForm() {
    ...
    document.getElementById('ptn-major-options').innerHTML = '<div class="dropdown-option no-results">Pilih Universitas terlebih dahulu</div>';
}
...
function handlePtnSave() {
    ...
    if (!uni || !major) {
        alert("Silakan pilih universitas dan jurusan terlebih dahulu.");
        return;
    }
    ...
    if (isDuplicate) {
        alert("Jurusan ini sudah kamu pilih di slot lain. Silakan pilih jurusan atau kampus yang berbeda.");
        return;
    }
...
function populateMajorDropdown(uniName) {
    ...
    if (!uniName) {
        container.innerHTML = '<div class="dropdown-option no-results">Pilih Universitas terlebih dahulu</div>';
        return;
    }
    ...
    if (majors.length === 0) {
        container.innerHTML = '<div class="dropdown-option no-results">Tidak ada jurusan yang sesuai filter</div>';
    } else {
...
function generateRecommendation() {
    ...
    vocational.forEach((item, idx) => {
        ...
        finalOrder.push({
            label: `Pilihan ${slotNum}`,
            item: item,
            desc: `Program Vokasi (${item.major.includes('D3') ? 'D3' : 'D4/Vokasi'})`
        });
    });
        ...
        if (!hasTarget) {
            warningHtml = `
                <div style="margin-top: 4px; display: flex; align-items: center; gap: 4px;">
                    <span style="color: #666; font-size: 0.75rem; font-weight: 600;">
                        <i class="fa-solid fa-circle-question"></i> Belum bisa di prediksi
                    </span>
                </div>
            `;
            ...
            warningHtml = `
                <div style="margin-top: 4px; display: flex; align-items: center; gap: 4px;">
                    <span style="color: #C62828; font-size: 0.75rem; font-weight: 600;">
                        <i class="fa-solid fa-triangle-exclamation"></i> Perlu Peningkatan
                    </span>
            ...
            warningHtml = `
                <div style="margin-top: 4px; display: flex; align-items: center; gap: 4px;">
                    <div style="font-size:0.6rem; color: white; background: #4caf50; padding: 2px 10px; border-radius: 12px; font-weight: 600; white-space: nowrap; width: fit-content;">Skor mencukupi</div>
                </div>
            `;
        }

        div.innerHTML = `
            <div class="rec-number">${rec.label.split(' ')[1]}</div>
            <div class="rec-text" style="flex: 1;">
                ...
                <div style="font-size: 0.75rem; color: #666; font-weight: 400;">
                    ${rec.desc} &bull; Target Minimum Skor: ${hasTarget ? rec.item.minScore : 'Belum ada data'}
                </div>
                ${warningHtml}
            </div>
            <button class="rec-delete-btn" title="Hapus" onclick="removePtnChoice(${rec.item.originalIndex})">
                <i class="fa-solid fa-trash-can"></i>
            </button>
        `;
    ...
    if (filledChoices.length < 4) {
        strategyTips.push("Maksimalkan 4 pilihan");
    }

    if (filledChoices.length >= 3) {
        if (vocational.length === 0) {
            strategyTips.push("Wajib min. 1 prodi Vokasi (D3/D4)");
        } else if (filledChoices.length === 4 && academic.length > 2) {
            strategyTips.push("Maks. 2 prodi Sarjana (S1)");
        }
        // Restore the specific note requested by the user
        alertItems.push(`<i class="fa-solid fa-circle-question"></i> <strong>Catatan:</strong> Jika memilih 4 prodi, Pilihan 3 dan 4 wajib program studi jenjang D3 atau D4.`);
    }

    if (strategyTips.length > 0) {
        alertItems.push(`<i class="fa-solid fa-lightbulb"></i> <strong>Saran Strategi:</strong> ${strategyTips.join(' • ')}`);
    }

    // Always add data accuracy disclaimer
    alertItems.push(`<i class="fa-solid fa-circle-info"></i> Data ini hanya <strong>perkiraan</strong> dan <strong>tidak dapat</strong> dijadikan acuan pada penerimaan sesungguhnya.`);

    if (alertItems.length > 0) {
        summaryEl.textContent = "Berdasarkan analisis skor dan keketatan, berikut rekomendasi urutan prioritas:";
        ...
        let itemsHtml = alertItems.map(item => {
            ...
            return `
                <div style="display:flex; align-items:flex-start; gap:10px; margin-bottom:6px; font-size:0.85rem; color:#4E342E; line-height:1.4;">
...
function switchRankingMode(mode) {
    ...
    if (searchInput) {
        searchInput.placeholder = mode === 'ptn' ? 'Cari kampus favoritmu...' : 'Cari prodi impianmu...';
...
function renderRankingList() {
    ...
    if (displayItems.length === 0) {
        container.innerHTML = '<div style="text-align:center; padding: 20px; color:#888;">Data tidak ditemukan</div>';
    } else {
            ...
            if (currentRankingMode === 'ptn') {
                ...
                div.innerHTML = `
                    ...
                    <div class="top-ptn-name" style="display:flex; flex-direction:column; gap:2px;">
                        ...
                        <span style="font-size:0.8rem; color:#666;">${item.university}</span>
                    </div>
...
window.onclick = function (event) {
    ...
    if (event.target == altModal) closeAltModal();
}
